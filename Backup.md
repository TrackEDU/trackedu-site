...
let authFailed = false; // ⚡ THE FIX: Global kill switch
...
function revealApp() {
  if (authFailed) return; // ⚡ THE FIX: Stop the reveal if they are locked out!
...
function showHardLock(reason) {
  authFailed = true; // ⚡ THE FIX: Flip the kill switch immediately
  
  if (tipInterval) clearInterval(tipInterval);
  localStorage.removeItem(CACHE_KEY);
  document.cookie = `${CACHE_KEY}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;`;
  
  // ⚡ THE FIX: Destroy the iframe so it can't continue doing background tasks
  const frame = document.getElementById('appFrame');
  if (frame) frame.remove(); 

  const welcomeScreen = document.getElementById('welcomeScreen');
  welcomeScreen.style.opacity = '1'; // Ensure it forces full visibility
  
  // 🎨 3X BETTER UI: Branded, clear, and actionable
  welcomeScreen.innerHTML = `
    <div class="executive-card" style="border-top: 6px solid #ef4444; padding: 40px 30px;">
      
      <div style="width: 64px; height: 64px; border-radius: 50%; background: #fef2f2; color: #ef4444; display: flex; align-items: center; justify-content: center; font-size: 28px; margin-bottom: 20px; box-shadow: 0 4px 10px rgba(239, 68, 68, 0.15);">
        <i class="fas fa-shield-alt"></i>
      </div>
      
      <h1 class="welcome-title" style="color:#0f172a; font-size: 22px; margin-bottom: 12px;">TrackEDU Access Paused</h1>
      
      <p style="font-size:14px; line-height:1.6; color:#475569; margin-bottom:20px;">
        Your school's connection to the TrackEDU platform is currently inactive. This is typically due to a pending account verification or a lapsed license.
      </p>
      
      <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px; width: 100%; margin-bottom: 25px; text-align: left;">
        <span style="font-size: 11px; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px;">System Diagnostic</span>
        <div style="font-size: 13px; font-weight: 600; color: #ef4444; margin-top: 4px;">
          ${reason || "Account access update required."}
        </div>
      </div>

      <div style="display:flex; flex-direction: column; gap:12px; width:100%;">
        <a href="mailto:hello@trackedu.net?subject=Account%20Verification%20Needed%20-%20School%20ID:%20${schoolId}" 
           style="width: 100%; box-sizing: border-box; background:#1e40af; color:#fff; text-decoration:none; padding:16px; border-radius:12px; font-weight:700; display:flex; align-items:center; justify-content:center; font-size:15px; box-shadow: 0 4px 12px rgba(30, 64, 175, 0.2); transition: transform 0.2s;">
          <i class="fas fa-envelope" style="margin-right: 8px;"></i> Email hello@trackedu.net
        </a>
        
        <button onclick="window.location.reload()" class="btn-launch" 
                style="background:#ffffff; color:#475569; box-shadow:none; border:2px solid #e2e8f0; padding: 14px; font-size: 14px;">
          <i class="fas fa-sync-alt" style="margin-right: 6px;"></i> Refresh Connection
        </button>
      </div>
